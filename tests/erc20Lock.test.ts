    ...
    describe('Lock Protection Tests', () => {
        describe('Lock Balances Check', () => {
            ...
            it('Should maintain the same total balance after locking tokens', async () => {
                ...
                const initialLockedBalance = await contract
                    .connect(deployer)
                    .getLockedAmount(deployer.address)
                const initialUnlockedBalance = await contract
                    .connect(deployer)
                    .getUnlockedAmount(deployer.address)
                ...
                const finalLockedBalance = await contract
                    .connect(deployer)
                    .getLockedAmount(deployer.address)
                const finalUnlockedBalance = await contract
                    .connect(deployer)
                    .getUnlockedAmount(deployer.address)
        ...
        describe('Lock Function Validation', () => {
            it('should revert when unlock block number is less than or equal to current block number', async () => {
                ...
                const currentBlockNumber = await ethers.provider.getBlockNumber()
                expect(
                    contract.connect(deployer).lockAmount(10, currentBlockNumber),
                ).to.be.revertedWith('Invalid Unlock Block Number')
                expect(
                    contract.connect(deployer).lockAmount(10, currentBlockNumber - 1),
                ).to.be.revertedWith('Invalid Unlock Block Number')
            })
            ...
            it('should revert when the user does not have enough unlocked balance to lock', async () => {
                ...
                const unlockBlockNumber = currentBlockNumber + 100
                const lockedAmount =
                    (await contract.connect(deployer).getUnlockedAmount(deployer.address)) + 1
            ...
            it('Should not allow locking tokens when the unlocked balance is insufficient', async () => {
                ...
                await expect(contract.connect(deployer).lockAmount(20, 110)).to.be.revertedWith(
